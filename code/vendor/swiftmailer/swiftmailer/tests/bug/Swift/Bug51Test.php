...
class Swift_Bug51Test extends \SwiftMailerTestCase
{
	...
	public function testAttachmentsDoNotGetTruncatedUsingToByteStream()
	{
		...
		for ($i = 0; $i < 10; ++$i) {
			$message = $this->_createMessageWithRandomAttachment(10000, $this->_attachmentFile);

			file_put_contents($this->_outputFile, '');
			$message->toByteStream(new Swift_ByteStream_FileByteStream($this->_outputFile, true));

			$emailSource = file_get_contents($this->_outputFile);

			$this->assertAttachmentFromSourceMatches(file_get_contents($this->_attachmentFile), $emailSource);
		}
	}
	...
	private function _createMessageWithRandomAttachment($size, $attachmentPath)
	{
		...
		$message = Swift_Message::newInstance()->setSubject('test')->setBody('test')->setFrom('a@b.c')->setTo('d@e.f')->attach(Swift_Attachment::fromPath($attachmentPath));
