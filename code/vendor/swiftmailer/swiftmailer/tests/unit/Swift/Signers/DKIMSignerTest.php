...
class Swift_Signers_DKIMSignerTest extends \SwiftMailerTestCase
{
    ...
    private function _createHeaders()
    {
        ...
        $headers->shouldReceive('getAll')->zeroOrMoreTimes()->with('From')->andReturn(array($headerFactory->createMailboxHeader('From', 'test@test.test')));
        $headers->shouldReceive('has')->zeroOrMoreTimes()->with('To')->andReturn(true);
        $headers->shouldReceive('getAll')->zeroOrMoreTimes()->with('To')->andReturn(array($headerFactory->createMailboxHeader('To', 'test@test.test')));
